...
// Cached APVTS parameter pointers + per-block runtime state shared across the
// engine, voices, arpeggiator, and FX chain. Voices and helpers read these
// atomics without locking; the audio thread owns modSum and the live MIDI
// values for the duration of each block.
struct SynthParams
{
    struct OscP {
        std::atomic<float>* on{};
