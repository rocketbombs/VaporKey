...
float FxChain::distort (float x, int type, float drive)
{
...
void FxChain::process (juce::AudioBuffer<float>& buffer, const SynthParams& params, double bpm)
{
    ...
    // Stereo delay (ping-pong-ish), tempo sync optional
    {
        ...
        if (dMix > 0.001f)
        {
            ...
            if (*params.delaySync > 0.5f)
            {
                ...
                dt = (float) (beats * 60.0 / juce::jmax (20.0, bpm));
